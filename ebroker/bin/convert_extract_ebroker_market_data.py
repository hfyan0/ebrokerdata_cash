...
def printDepthInfo(currID, updatetime):
  ...
  ###################################################
	try:
		global stkCodePadLeadingZero
		global useFullTimestamp
		stock_code = currID.split("_")[2]
		outstr = ""

		if (stkCodePadLeadingZero == "true") or (stkCodePadLeadingZero == "True") or (stkCodePadLeadingZero == "T"):
			if (len(stock_code) < 5):
				stock_code = stock_code.zfill(5)
		else:
			stock_code = str(int(stock_code))

		if (useFullTimestamp == "true") or (useFullTimestamp == "True") or (useFullTimestamp == "T"):
			updatetime = time.strftime("%Y:%m:%d") + "_" + updatetime + "_000000"

		outstr = "%s,%s,%s,%s,"%(updatetime.replace(":",""), stock_code,currPrices[currID]["lastPrice"],currPrices[currID]["accSize"])
	except:
...
#			updateInfo = updateInfo + "last_traded %s " %(fields[i+1])
	...
	if (len(updateInfo)>0):
		...
		if startTime < currTime and endTime > currTime:
			...
			return printDepthInfo(currID, updatetime) # + ","+ str(deal_src_id) + "," + str(hasOMSTrade)
		else:
			return ""
	else:
		return ""
