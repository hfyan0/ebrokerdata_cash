...
#	rawEBrokerFile_file = open(rawEBrokerFilename_str, "w")
#	parsedMarketDataFile_file = open(parsedMarketDataFilename_str, "w")
...
def connectEBroker(server, port, feedcodes, processEbrokerUpdate):
	...
	for line in atu_tcp_client.non_blocking_readlines(sock, isMarketOpenNow):
		line = line.strip()
		ret=processEbrokerUpdate(line)
		yield ret
def getfeedstream():
	print "--- Start ---"
	init()	
	yield connectEBroker(para_ebroker_server_address_str, int(para_ebroker_server_port_str), para_feedcodes_str, processEbrokerUpdate)
	print "LunchTime"
	waitUntilMarketOpen()
	yield connectEBroker(para_ebroker_server_address_str, int(para_ebroker_server_port_str), para_feedcodes_str, processEbrokerUpdate)
